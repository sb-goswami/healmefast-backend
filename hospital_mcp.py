...
if not MONGODB_URI or "mongodb" not in MONGODB_URI:
    print("[MCP WARNING] MONGODB_URI is missing or invalid. Tools will be unavailable.")
    client = None
    ...
    try:
        ...
        print(f"[MCP ERROR] Could not initialize MongoDB: {e}")
        client = None
        doctors_collection = None
        appointments_collection = None
...
def search_doctors(specialty: str) -> str:
    ...
    if doctors_collection is None:
        return "ERROR: Doctor database service is currently unavailable. Please check the backend configuration (MONGODB_URI)."
    ...
    try:
        ...
        return f"ERROR: Failed to search doctors. {str(e)}"
...
def get_doctor_slots(doctor_selection: str) -> str:
    ...
    if appointments_collection is None:
        return "ERROR: Appointment service is currently unavailable."
    ...
    try:
        ...
        return f"ERROR: Failed to retrieve slots. {str(e)}"
...
def book_appointment(doctor_selection: str, time_slot: str) -> str:
    ...
    if appointments_collection is None:
        return "ERROR: Booking service is currently unavailable."
